...
namespace EnumDevicesApp
{
    ...
    public class TcpServer
    {
        ...
        public TcpServer(TcpServiceProvider provider, int port)
        {
            _provider = provider;
            _port = port;
            serverAddr = new IPEndPoint(IPAddress.Broadcast, 9000);
            _listener = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
            _listener.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.Broadcast, 1);
            _listener.Connect(serverAddr);
        ...
        public bool Start()
        {
            try
            {
                ...
                // Set the send buffer size to 2 bytes.
                //_listener.SendBufferSize = 16;

                // Set the timeout for synchronous send methods 
                // to 1 second (500 milliseconds.)			
                //_listener.SendTimeout = 500;
                //_listener.Bind(new IPEndPoint(IPAddress.Parse("192.168.1.65"), _port));
        ...
        private void ConnectionReady_Handler(IAsyncResult ar)
        {
            lock (this)
            {
                ...
                //Socket conn = _listener;
                if (_connections.Count >= _maxConnections)
                {
                    //Max number of connections reached.
                    string msg = "SE001: Server busy";
                    //_listener.Send(Encoding.UTF8.GetBytes(msg), 0, msg.Length);
        ...
        public void SendToAll(byte[] buffer)
        {
            lock (this)
            {
                _listener.Send(buffer);
                //_listener.SendTo(buffer, serverAddr);
                /*foreach (object obj in _connections)
                {
                    ConnectionState st = obj as ConnectionState;
